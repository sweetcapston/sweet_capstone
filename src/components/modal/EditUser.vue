...
export default {
  data() {
    return {
          ...
          ? colorList[
              parseInt(
                this.$store.state.studentID[
                  this.$store.state.studentID.length - 1
                ]
              )
            ]
          : "gradient",
      ...
      classCode: this.$store.state.classCode,
  ...
  created() {
    this.$EventBus.$on("editUser", () => {
      ...
      document.querySelector(
        ".password .v-input__control .v-input__slot .v-text-field__slot input"
      ).disabled = false;
    });
  },
